$(function () {
  ...
  function _initilize() {
    ...
    $(".question-item .form-check")
      .find("label")
      .on("keypress keyup paste cut", function () {
    ...
    $(".choice-option").change(function () {
      ...
      // $("select option[value='" + choice + "']").attr("selected", "selected");
      ...
      if (choice == "p") {
        ...
        $(this)
          ...
          .find(".choice-field")
          .html(
            '<button type="button" class="add_chk btn btn-sm btn-default border"><i class="fa fa-plus"></i> Add option</button>'
          );
        add_checkbox();
        for (var i = 0; i < 3; i++) {
        ...
        $(this)
          ...
          .find(".choice-field")
          .html(
            '<button type="button" class="add_radio btn btn-sm btn-default border"><i class="fa fa-plus"></i> Add option</button>'
          );
        add_radio();
        for (var i = 0; i < 3; i++) {
  ...
  function save_form() {
    ...
    var form_code =
      $("[name='form_code']").length > 0 ? $("[name='form_code']").val() : "";
    var title = $("#form-title").text();
    var description = $("#form-description").text();
    var created_by = $("#created_by").text();
    var datacount = form_el.find("[data-item]").length;
    ...
    $.ajax({
      ...
      success: function (resp) {
        if (typeof resp == "object" && resp.status == "success") {
          ...
          window.form_code_id = resp.form_code;
        } else {
          console.log(resp);
          alert("an error occured");
        }
    ...
    var form_code_id = window.form_code_id;
    for (let i = 0; i < datacount; i++) {
      var datait = $('[data-item="' + i + '"]').clone();
      var datop = datait.find('[name="choice"]').val();
      var itno = i;
      $.ajax({
        url: "classes/Forms.php?a=save_meta",
        method: "POST",
        data: {
          form_code: form_code_id,
          meta_code: itno,
          meta_type: datop,
        },
        dataType: "json",
